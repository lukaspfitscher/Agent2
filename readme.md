...
- or just paste this in your terminal `apt update; apt install -y python3 python3-pip python3-requests`.
- Add your Openrouter API key in the agent2.py file.
...
- After that the script gets executed in a separate shell. 
- It runs in a separate shell and therefore doesn't block the agent.
...
├─ agent2.py          # Contains config + single python file (whole code)
...
First install the software you wa as usual:
