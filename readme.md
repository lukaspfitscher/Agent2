...
- Everything after the last `agent2_script_start` is interpreted as a bash script
- Before every execution the script is reset (path doesn't persist, no environment variables persist).
...
- If the model doesn't request another script, the user is prompted for further instructions.
